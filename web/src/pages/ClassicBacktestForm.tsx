...
import type { BacktestRequest, BacktestResponse, OverrideInput, StrategyTemplate } from "../types";
...
// Sizing surface shared by the margin-based strategies (owner, 2026-07-17):
// margin_per_lot = TODAY'S broker margin (₹) for ONE lot-set of the strategy's full
// structure (straddle CE+PE pair ≈ ₹2L; a credit spread ≈ ₹50k) — scaled back in time by
// notional so 2021 margins shrink with 2021 spots/lot-sizes; 0 = the model margin.
...
const INTRADAY_DEFAULTS: Record<string, Record<string, number | string | boolean>> = {
  ...
  delta_neutral_monthly: { lots: 1, target_delta: 0.18, profit_target_pct: 2.5,
    stop_loss_pct: 0, force_entry: true, ironfly_adjust: false, ...SIZING_FIELDS },
 ...
 *  call+put_ratio_monthly). Left structurally as-is on purpose: its per-strategy default
 *  effects and the template-apply effect are ORDER-SENSITIVE (CLAUDE.md §9), so the v2
 *  work lifts only `strategyId` + the strategy lists out to the dispatcher and touches
 *  nothing else. */
export default function ClassicBacktestForm({ embedded = false, strategyId, onStrategyChange,
  strategies, eodList, intradayList }: {
  embedded?: boolean;
  strategyId: string;
  ...
  const setStrategyId = onStrategyChange;
  ...
  const mutation = useMutation({
    // EOD only — the INTRADAY basis goes through the background-job flow (startReplay +
    // the progress poll below) so long replays get a progress bar instead of one
    // silent multi-minute request.
    mutationFn: (body: BacktestRequest) => api.backtest(body),
  });
  // INTRADAY replay job: id + the body that started it. replayBody survives job
  // completion (the Save flow echoes it, like mutation.variables does for EOD).
  const [replayJob, setReplayJob] = useState<{ id: string; body: BacktestRequest } | null>(null);
  const [replayBody, setReplayBody] = useState<BacktestRequest | null>(null);
  const [replayResult, setReplayResult] = useState<BacktestResponse | null>(null);
  const [replayError, setReplayError] = useState<string | null>(null);
  const { data: jobSnap } = useQuery({
    queryKey: ["replay-progress"],
    queryFn: api.backtestIntradayProgress,
    enabled: replayJob != null,
    refetchInterval: replayJob != null ? 1500 : false,
  });
  useEffect(() => {
    if (replayJob == null || jobSnap == null || jobSnap.id !== replayJob.id) return;
    if (jobSnap.status === "done" && jobSnap.result) {
      setReplayResult(jobSnap.result);
      setReplayJob(null);
    } else if (jobSnap.status === "error") {
      setReplayError(jobSnap.error ?? "replay failed");
      setReplayJob(null);
    }
  }, [jobSnap, replayJob]);
  ...
  async function startReplay() {
    // INTRADAY runs are a background JOB (a 5-year window replays for minutes): POST
    // returns {job_id}; the progress query below polls to done and adopts the result.
    const body = { ...buildIntradayBody(), persist: false };
    setReplayResult(null);
    setReplayError(null);
    setReplayBody(body);
    try {
      const { job_id } = await api.backtestIntraday(body);
      setReplayJob({ id: job_id, body });
    } catch (e) {
  ...
  const result = isIntraday ? replayResult : mutation.data;
    ...
    <div className="space-y-6">
      ...
      <Panel className="p-5 max-w-[760px]">
        <form onSubmit={submit} className="space-y-4">
            ...
            <>
              ...
              <div className="grid md:grid-cols-4 gap-4">
                ...
                {Object.entries(iparams)
                  // Under capital sizing the manual lots/sets count is IGNORED (recomputed
                  // from equity each flat day) — showing it only invites "which one wins?".
                  .filter(([k]) => !(iparams.sizing === "capital" && (k === "lots" || k === "sets")))
                  .map(([k, v]) => (
              ...
              {iparams.sizing === "capital" && (
                Number(iparams.margin_per_lot) > 0 ? (
                  <div className="text-[12px] text-[var(--muted)]">
                    ≈ <b className="text-[var(--strong)]">
                      {Math.max(0, Math.floor(capital / (Number(iparams.margin_per_lot)
                        * (1 + Number(iparams.sizing_buffer_pct || 0) / 100))))} lots today
                    </b>{" "}
                    — capital ÷ (margin/lot × {(1 + Number(iparams.sizing_buffer_pct || 0) / 100)
                      .toFixed(2)} buffer). Recomputed each flat day from CURRENT equity and
                    era-scaled back in time (older years → smaller margins → more lots).
                  </div>
                ) : (
                  <div className="text-[11px] text-amber-700 dark:text-amber-300/90">
                    sizing=capital needs <b>margin per lot</b> — key today's broker margin (₹)
                    for one lot-set of this strategy's structure.
                  </div>
                )
              )}
              <div className="text-[11px] text-[var(--faint)]">
                ...
                stops use the MODEL margin (~1.5-2× broker) unless you key{" "}
                <b>margin_per_lot</b>: TODAY'S broker margin (₹) for one lot-set of this
                strategy's structure (straddle pair ≈ ₹2L, credit spread ≈ ₹50k) — scaled back
                in time by notional, so old years use proportionally smaller margins.{" "}
                <b>sizing=capital</b> then refits lots each flat day:
                floor(equity ÷ (era margin × (1 + buffer%))). Lot sizes are era-true
                (NIFTY 50→25→75→65, BANKNIFTY 25→15→30→35 per NSE revisions). Every new
                captured day extends the window.
              </div>
            </>
          )}
          ...
          <button
            type="submit"
            disabled={mutation.isPending || sweepProgress != null || replayJob != null}
          ...
          {/* The replay job's live progress (a 5-year window is minutes of replay) —
              leaving this page is harmless; returning re-attaches via the same poll. */}
          {replayJob != null && jobSnap?.status === "running" && (jobSnap.total ?? 0) > 0 && (
            <div className="mt-3 max-w-md">
              <div className="text-xs text-[var(--muted)] tabular-nums">
                Replaying {jobSnap.day ?? "…"} · day {Math.min(jobSnap.done + 1, jobSnap.total)}/
                {jobSnap.total} · {Math.round((100 * jobSnap.done) / jobSnap.total)}%
              </div>
              <div className="mt-1 h-[7px] rounded-full bg-[var(--chip)] overflow-hidden">
                <div className="h-full rounded-full bg-[var(--ft)]"
                  style={{ width: `${(100 * jobSnap.done) / jobSnap.total}%` }} />
              </div>
            </div>
          )}
        </form>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {result.run_id != null ? (
              ...
              <>
                ...
                <button
                  onClick={() => {
                    const reqBody = isIntraday ? replayBody : mutation.variables;
                    if (!reqBody) return;
                    ...
                    saveMutation.mutate({
                      request: { ...reqBody, name: name.trim() || undefined, notes: notes.trim() || undefined },
