...
export const DEPLOY_REGISTRY: DeploySpec[] = [
  ...
  },
  {
    ...
    group: "monthly", cadence: "MONTHLY",
    blurb: "Sell 2 ATM, buy 1 each side on the monthly — defined risk, no adjustments",
    instrument: "DERIV", needsBroker: true,
    underlyings: ["BANKNIFTY", "NIFTY"],
    route: "/trade/options/monthly-butterfly/deploy",
    fields: [
      f("sets", "Lot-sets", "number", 1,
        { hint: "scale in: 1 set for one cycle first, then 5, 10, 20 — measure slippage at each" }),
      MARGIN_PER_SET(70000, "measure the 3 legs on Kite's basket calculator first"),
      f("side", "Side", "select", "ce", {
        options: [
          { value: "ce", label: "Calls (best in the 5-year replay)" },
          { value: "pe", label: "Puts" },
        ],
      }),
      f("wing_points", "Wing width (pts)", "number", 400, { step: "any",
        hint: "400 on BANKNIFTY: best in both halves of the 5-year replay; the debit (~₹7.5k per 10 sets) is large enough that spread noise stops mattering" }),
      ...
      f("cycle", "Cycle", "select", "monthly", {
        options: [
          ...
          { value: "weekly", label: "Weekly expiry (worse everywhere in the replay)" },
        ],
      }),
  ...
  },
  {
    ...
    name: "Directional condor",
    group: "monthly", cadence: "DAILY SUPERTREND · 09:30",
    blurb: "NOT YET FORWARD-TESTED · long CE/PE condor on the daily SuperTrend flip, profit-lock wing walk",
    instrument: "DERIV", needsBroker: true,
    underlyings: ["NIFTY"],
    fields: [
      f("lots", "Lots", "number", 1),
      f("st_multiplier", "SuperTrend multiplier (period 11)", "number", 2.9, { step: "any" }),
      f("confirm_bars", "Confirm bars", "number", 1),
      f("width_pct", "Segment width (% of spot)", "number", 1, { step: "any",
        hint: "each strike gap, rounded to the 100 grid — 1% ≈ 200 pts today" }),
      f("lock_start_pct", "Profit lock from (% of anchor)", "number", 2, { step: "any" }),
      f("lock_step_pct", "Lock step (% of anchor)", "number", 1, { step: "any" }),
      f("half_loss_pct", "Stop (% of max loss)", "number", 50, { step: "any" }),
      TIME("entry_time", "Entry time", "09:30"),
      f("roll_days_before", "Roll days before expiry", "number", 5),
      MARGIN_PER_SET(100000, "A hedged long condor's broker margin is ≈ the debit (~₹4-5k), so a % of it fires on noise — the deck's returns imply ~₹1L of capital per lot"),
      ...cadence("1min", "1min"),
      MARK_BASIS,
      FORCE,
    ],
    warn: "Not forward-tested — paper first. A fresh deploy inside a trend WAITS for the next daily flip unless you force it (force takes the current side).",
  },
  {
